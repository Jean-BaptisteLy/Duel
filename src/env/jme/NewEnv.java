...
public class NewEnv extends SimpleApplication {
	...
	public synchronized boolean shoot(String agent, String enemy) {
		if (players.containsKey(agent) && players.containsKey(enemy)) {
			...
			if (isVisible(agent, enemy, MAX_DISTANCE)) {
				...
				// TP 2 Question 3)c
				if(agent.equals("Player1")) {
					//System.out.println(agent+" : Adversaire visible !(NewEnv.java)");
					savePercoitSonAdversaireCSV();
				}
				...
				if ( r.nextFloat() < impact){
					...
					if(enemy.equals("Player1")){
						//System.out.println(enemy+" : Je suis touché !(NewEnv.java)");
						//System.out.println(enemy+" life : "+enemyAgent.life+" (NewEnv.java)");
						saveEstToucheCSV();
					}
	...
	public static void saveCSV(){ // TP 2 Question 3)b et 3)c
		
		String res = PrologBehavior.sit.toCSVFile();
		int id = new Random().nextInt(10000);
		System.out.println(res);
		String[] arrOfStr = res.split(";", 14); 
		try{
			if (arrOfStr[arrOfStr.length - 1].equals("VICTORY")) { // 3)b
				PrintWriter writer = new PrintWriter(System.getProperty("user.dir")+"/ressources/learningBase/victory/Mosimu_"+id+".csv", "UTF-8");
				writer.println(res);
			    writer.close();
			    System.out.println("Execution result saved in /ressources/learningBase/victory/");
			}
			else if (arrOfStr[arrOfStr.length - 1].equals("DEFEAT")) {
				// DEFEAT 3)b
				PrintWriter writer = new PrintWriter(System.getProperty("user.dir")+"/ressources/learningBase/defeat/Mosimu_"+id+".csv", "UTF-8");
				writer.println(res);
			    writer.close();
			    System.out.println("Execution result saved in /ressources/learningBase/defeat/");
			}
			else {
				System.out.println("Execution result not saved !!!");
			}
			// simus
			/*
