...
public class HighestExploreBehavior extends TickerBehaviour {
	...
	public HighestExploreBehavior(Agent a, long period) {
		super(a, period);
		...
		prlNextOffend = true;
		//System.out.println("Player HighestExplore");
	}
	...
	protected void onTick(){
		...
		if (target == null && !setTarget()){ 
			System.out.println("Ma target est null ! Je me déplace aléatoirement.");
			randomMove();
			return;
		}
		...
		if (agent.getCurrentPosition().distance(target) < AbstractAgent.NEIGHBORHOOD_DISTANCE){
			Vector3f nei;
			double rand = Math.random(); // TP 2 Question 3)a
			if (rand <= 0.91) {
				System.out.println("Je vais vers l'endroit le plus haut perçu dans mon champ de vision avec une probabilité de 0.91 .");
				nei = findHighestNeighbor();
				//System.out.println("---------- NEI : "+nei+" ----------");
			}
			else {
				System.out.println("Je vais vers un point d'intérêt avec une probabilité de 0.09 .");
			...
			if(nei != null){
				...
				System.out.println("Target null : Random move");
				addInterestPoint();
				target = null;
				//targetType = null;
				//randomMove();
